...
export const SelectCategoryCitation = ({
  setQuotes,
  isLoading,
}: {
  setQuotes: React.Dispatch<
    React.SetStateAction<TypeQuotesResponseSchema | undefined>
  >;
  isLoading: boolean;
}) => {
        ...
        {BUTTONS_QUOTES.slice(0, displayedBtnQuote).map((btnQuote, index) => (
          <ButtonQuote
            ...
            handleChangeCategoryQuote={() =>
              handleChangeCategoryQuote(btnQuote.content)
            }
            key={index}
            content={btnQuote.content}
            keyword={btnQuote.keyword}
          />
        ))}
      </div>
      <div className="mt-8">
